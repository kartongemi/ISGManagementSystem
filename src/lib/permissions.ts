...
/**
 * Kullanıcının uygunsuzluk kaydı oluşturma yetkisi var mı?
 */
export function canCreateNonConformance(user: User) {
  return user.role === 'center_manager' || 
         user.role === 'project_manager' || 
         user.role === 'isg_specialist'
}
...
/**
 * Kullanıcının belirli bir uygunsuzluğu görüntüleme yetkisi var mı?
 */
export function canViewNonConformance(user: User, nonConformance: any) {
  // Merkez yöneticisi tüm uygunsuzlukları görebilir
  if (user.role === 'center_manager') return true
  
  // Proje yöneticisi atandığı projelerdeki uygunsuzlukları görebilir
  if (user.role === 'project_manager') {
    return user.projectIds?.includes(nonConformance.projectId) || false
  }
  
  // İSG uzmanı şirketinin projelerindeki uygunsuzlukları görebilir
  if (user.role === 'isg_specialist') {
    const userProjects = getUserProjects(user)
    return userProjects.some((p) => p.id === nonConformance.projectId)
  }
  
  // Taşeron yöneticisi sadece kendisine yöneltilmiş uygunsuzlukları görebilir
  if (user.role === 'contractor_manager') {
    return nonConformance.companyId === user.companyId
  }
...
/**
 * Kullanıcının belirli bir uygunsuzluğu kapatma yetkisi var mı?
 */
export function canCloseNonConformance(user: User, nonConformance: any) {
  // Sadece uygunsuzluğun yöneltildiği şirketin kullanıcıları kapatabilir
  return nonConformance.companyId === user.companyId && 
         (user.role === 'contractor_manager' || user.role === 'center_manager')
}

/**
 * Kullanıcının erişebildiği uygunsuzlukları döndürür
 */
export function getUserNonConformances(user: User, allNonConformances: any[]) {
  if (user.role === 'center_manager') {
    return allNonConformances
  }
  
  if (user.role === 'project_manager') {
    const userProjectIds = new Set(user.projectIds || [])
    return allNonConformances.filter((nc) => userProjectIds.has(nc.projectId))
  }
  
  if (user.role === 'isg_specialist') {
    const userProjects = getUserProjects(user)
    const projectIds = new Set(userProjects.map((p) => p.id))
    return allNonConformances.filter((nc) => projectIds.has(nc.projectId))
  }
  
  if (user.role === 'contractor_manager') {
    // Taşeron sadece kendisine yöneltilmiş uygunsuzlukları görür
    return allNonConformances.filter((nc) => nc.companyId === user.companyId)
  }
