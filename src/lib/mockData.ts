...
// ============================================
// SAHA DENETİM & GÖZLEM (UYGUNSUZLUK YÖNETİMİ)
// ============================================

export type NonConformanceType = 'hazardous_condition' | 'hazardous_behavior'
export type NonConformanceStatus = 'open' | 'in_progress' | 'closed'

export interface NonConformance {
  id: string
  projectId: string
  companyId: string              // Uygunsuzluğun yöneltildiği firma
  reportedByCompanyId: string    // Uygunsuzluğu raporlayan firma
  reportedByUserId: string
  reportedAt: string
  
  // Uygunsuzluk detayları
  type: NonConformanceType
  standard?: string              // Standart/mevzuat gerekliliği
  description: string
  recommendedAction: string
  
  // Dosyalar (mock URL'ler)
  photos: string[]
  documents: string[]
...
function getDefaultNonConformances(): NonConformance[] {
  return [
    {
      id: 'nc-1',
      projectId: 'proj-1',
      companyId: 'comp-2',
      reportedByCompanyId: 'comp-1',
      reportedByUserId: 'user-3',
      reportedAt: '2025-01-02T10:30:00Z',
      type: 'hazardous_condition',
      standard: 'İSG Yönetmeliği Madde 5 - İş Ekipmanlarının Kullanımı',
      description: 'Şantiye alanında korkuluksuz yüksekte çalışma platformu tespit edildi. 3 metre yükseklikte bulunan iskele platformunda yan korkuluklar eksik.',
      recommendedAction: 'İskele platformlarına ISO 12100 standardına uygun yan korkuluklar monte edilmeli. Minimum 1 metre yüksekliğinde korkuluk ve ara korkuluk sistemi kurulmalı.',
      photos: ['/mock/nc/nc-1-photo1.jpg', '/mock/nc/nc-1-photo2.jpg'],
      documents: ['/mock/nc/nc-1-report.pdf'],
      status: 'closed',
      closureAction: 'Tüm iskele platformlarına üst korkuluk (1.1m), ara korkuluk (0.5m) ve süpürgelik (15cm) monte edildi. Bağlantı noktaları kontrol edildi ve sabitlendi.',
      closurePhotos: ['/mock/nc/nc-1-closure1.jpg', '/mock/nc/nc-1-closure2.jpg'],
      closureDocuments: ['/mock/nc/nc-1-closure-report.pdf'],
    ...
    },
    {
      ...
      reportedByCompanyId: 'comp-1',
      reportedByUserId: 'user-3',
      reportedAt: '2025-01-03T09:15:00Z',
      type: 'hazardous_behavior',
      standard: 'KKD Kullanımı Yönetmeliği',
      description: 'Kaynak işlemi sırasında 2 personelin kaynak maskesi kullanmadığı gözlemlendi. Gözleri koruyucu ekipman olmadan elektrik kaynağı yapılıyor.',
      recommendedAction: 'Tüm kaynak personeline uygun kaynak maskesi temin edilmeli ve kullanımı zorunlu tutulmalı. KKD kullanım eğitimi verilmeli.',
      photos: ['/mock/nc/nc-2-photo1.jpg'],
      documents: [],
      status: 'in_progress',
      closureAction: 'Kaynak maskeleri temin edildi ve personele dağıtıldı. KKD kullanım eğitimi planlandı (08 Ocak 2025).',
      closurePhotos: ['/mock/nc/nc-2-closure1.jpg'],
      closureDocuments: [],
    ...
    },
    {
      ...
      type: 'hazardous_condition',
      standard: 'Elektrik İç Tesisleri Yönetmeliği',
      description: 'Elektrik panosu kapağı açık durumda ve uyarı levhası yok. Pano içinde izolasyonu soyulmuş kablolar görülüyor.',
      recommendedAction: 'Elektrik panosu kapağı kapatılmalı ve kilitlenmeli. Uyarı levhası asılmalı. İzolasyonu bozuk kablolar yenilenmelidir.',
      photos: ['/mock/nc/nc-3-photo1.jpg', '/mock/nc/nc-3-photo2.jpg'],
      documents: ['/mock/nc/nc-3-electrical-report.pdf'],
      status: 'open',
    ...
    },
    {
      ...
      reportedByCompanyId: 'comp-1',
      reportedByUserId: 'user-3',
      reportedAt: '2025-01-04T08:30:00Z',
      type: 'hazardous_behavior',
      standard: 'Çalışma ve Sosyal Güvenlik Bakanlığı - Kişisel Koruyucu Donanım Yönetmeliği',
      description: 'Beton dökümü sırasında 3 işçinin baret kullanmadığı tespit edildi.',
      recommendedAction: 'Tüm çalışanlara baret kullanımı hatırlatılmalı ve denetim sıklaştırılmalıdır.',
      photos: ['/mock/nc/nc-4-photo1.jpg'],
      documents: [],
      status: 'open',
...
export const nonConformances: NonConformance[] = getDefaultNonConformances()

export function getNonConformances() {
  return nonConformances
}

export function getNonConformanceById(id: string) {
  return nonConformances.find((nc) => nc.id === id)
}

export function getNonConformancesByProject(projectId: string) {
  return nonConformances.filter((nc) => nc.projectId === projectId)
}

export function getNonConformancesByCompany(companyId: string) {
  return nonConformances.filter((nc) => nc.companyId === companyId)
}

export function getNonConformancesByStatus(status: NonConformanceStatus) {
  return nonConformances.filter((nc) => nc.status === status)
}
